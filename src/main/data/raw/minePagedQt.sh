	...
	do
		...
		then 
			...
			then
				...
				timeout 15m curl "https://codereview.qt-project.org/changes/?q=is:${status}&o=LABELS&o=DETAILED_LABELS&o=ALL_FILES&o=DETAILED_ACCOUNTS&o=ALL_REVISIONS&o=MESSAGES&n=${pageSize}" -o "qt_changes_${i}_${status}.json"
				calls=`expr $calls + 1`
			else
			...
			then
				...
				then
					...
					timeout 15m curl "https://codereview.qt-project.org/changes/?q=is:${status}&o=LABELS&o=DETAILED_LABELS&o=ALL_FILES&o=DETAILED_ACCOUNTS&o=ALL_REVISIONS&o=MESSAGES&N=${sortkey}&n=${pageSize}" -o "qt_changes_${i}_${status}.json"
					calls=`expr $calls + 1`
				else
					echo "Skipping page $i for Qt, already retrieved"
				fi
